...
election <- select(election,-c(year,version))
str(election)
```
...
covid2 <- arrange(covid2,desc(cases))
head(covid2,10)
```
...
```{r}
combine3 <- arrange(combine,desc(cases))
combine3 <- select(combine3,c(state,date,cases,deaths,party))
head(combine3,10)
```